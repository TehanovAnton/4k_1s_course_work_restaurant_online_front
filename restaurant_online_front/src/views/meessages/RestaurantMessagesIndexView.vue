<script setup>
  ...
  const message = ref({ text:"", restaurant_id: props.restaurant.id, user_id: currentUser.user.id })
  ...
  const getRestaurantMessages = async () => {
    let {
      response,
      isSuccessful
    } = await message_service.apiGetRestaurantMessges(props.restaurant.id)
  ...
  const postMessage = async (e) => {
    if (e.key == 'Enter') {
      ...
      let {
        response,
        isSuccessful
      } = await message_service.apiPostRestaurantMessges(props.restaurant.id, message.value)
  ...
  const deleteMessage = async (messageId) => {
    let {
      response,
      isSuccessful
    } = await message_service.apiDeleteRestaurantMessage(props.restaurant.id, messageId)
